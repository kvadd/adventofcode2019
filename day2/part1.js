...
function runPuzzle(inputArray, inputBit) {
    let output;
    ...
    if (inputBit[0] === 1) {
        output = inputArray[inputBit[1]] + inputArray[inputBit[2]];
    }
    else if (inputBit[0] === 2) {
        output = inputArray[inputBit[1]] * inputArray[inputBit[2]];
    }

    inputs[inputBit[3]] = output;
...
while(bitResult != 1) {
    const inputBits = [inputs[index1], inputs[index2], inputs[index3], inputs[index4]];
    bitResult = runPuzzle(inputs, inputBits);
    index1 += 4;
    index2 += 4;
    index3 += 4;
    index4 += 4;
