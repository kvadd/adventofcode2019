...
const OPERATIONS = {
    ...
    2: (param1, param2) => param1 * param2,
};

while(inputs[index] !== 99) {
    const instruction = [inputs[index], inputs[index + 1], inputs[index + 2], inputs[index + 3]];
    compute(inputs, instruction);
    index += 4;
}

function compute(memory, instruction) {
    const operation = OPERATIONS[instruction[0]];
    const param1 = memory[instruction[1]];
    const param2 = memory[instruction[2]];
    const param3 = instruction[3];

    let output = operation(param1, param2);
